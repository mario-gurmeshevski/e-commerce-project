...
import { MinusIcon, PlusIcon, TruckIcon, XMarkIcon } from '@heroicons/react/24/solid';
...
export default function Checkout() {
    ...
    const { cartItems, addToCart, removeFromCart, decreaseQuantity, clearCart } = useCart();    const navigate = useNavigate();
    ...
    const validateForm = () => {
        ...
        if (!phoneRegex.test(formData.phoneNumber)) {
            errors.phoneNumber = 'Телефонскиот број мора да започне со 07 и да има точно 9 цифри';
        }
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            console.error(err);
            alert('Настана грешка при праќањето на нарачката. Обидете се повторно.');
        } finally {
            setLoading(false);
        }
    };
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-medium text-black mb-2">Име</label>
                                        <input
                                            type="text"
                                            name="firstName"
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-black mb-2">Презиме</label>
                                        <input
                                            type="text"
                                            name="lastName"
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-black mb-2">Email адреса</label>
                                    <input
                                        type="email"
                                        name="email"
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-black mb-2">Адреса</label>
                                    <input
                                        type="text"
                                        name="address"
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-medium text-black mb-2">Град</label>
                                        <input
                                            type="text"
                                            name="city"
                                            value={formData.city}
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-black mb-2">Општина</label>
                                        <input
                                            type="text"
                                            name="municipality"
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-medium text-black mb-2">Поштенски број</label>
                                        <input
                                            type="text"
                                            name="postalCode"
                        ...
                        {cartItems.map((item) => (
                            <div key={item.id} className="flex items-center justify-between">
                                ...
                                <div className="flex items-center space-x-3">
                                    ...
                                    <span className="font-medium ml-2 min-w-[80px] text-right">
                                        {preciseRound(item.price * (1 - (item.discount || 0)/100) * item.quantity)} ден.
                                    </span>
                            </div>
                        </div>
                    ))}
