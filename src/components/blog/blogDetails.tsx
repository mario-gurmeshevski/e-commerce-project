...
import type { ContentItem, Blog, BlogSection } from '../../interfaces/blog.ts'
...
const BlogDetails = () => {
	...
	const storageKey = `currentBlog:${slug}`
	const [blogItem, setBlogItem] = useState<Blog | null>(null)
	const [loading, setLoading] = useState(!blogItem)
	...
	useEffect(() => {
		...
		fetchWithFallback(`${API_CONFIG.endpoints.blog}/${slug}`, `blog/${slug}.json`)
			.then((res: { data: any }) => {
				// Handle both array and single object responses
				let blogData: Blog | null = null
				if (Array.isArray(res.data)) {
					blogData = res.data.find((item: Blog) => item.slug === slug) || null
				} else {
					blogData = res.data as Blog
				}

				if (blogData) {
					setBlogItem(blogData)
					sessionStorage.setItem(storageKey, JSON.stringify(blogData))
					document.title = `${blogData.title} - E Commerce`
				} else {
					console.error(`Blog item with slug ${slug} not found in response`)
				}
			})
			.catch((error) => {
		...
		<div className="max-w-4xl md:max-w-7xl mx-auto py-10 px-4 min-h-screen">
			...
			<div className="rounded-xl p-6 md:p-8 mb-8">
				...
				<div className="flex items-center text-sm text-gray-500 mb-6">
					...
					{new Date(blogItem.createdAt).toLocaleDateString('mk-MK', {
						year: 'numeric',
						month: 'long',
						day: 'numeric',
					})}
			...
			<div className="space-y-8">
				{blogItem.content.map((section, index) => renderSection(section, index))}
			</div>
		</div>
	)
}
